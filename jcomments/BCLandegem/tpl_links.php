...
<?php
	...
	function getReadmoreLink() 
	{
		...
		return '<a class="comments-readmore btn btn-primary" href="'.$link.'" title="'.htmlspecialchars($title).'"> <i class="fa fa-angle-right fa-lg" style="margin-right: 5px"></i>'.$text.'</a>';
	}
	...
	function getCommentsLink()
	{
		...
		switch($style) {
			case -1:
				return $count > 0 ? '<span class= btn"'.$css.'">'.$text.'</span>' : '';
				break;
			default:
				return '<a class=" btn '.$css.'" href="'.$link.'" title="'.htmlspecialchars($text).'"><i class="fa fa-comment" style="margin-right: 5px"></i>'.$text.'</a>';
				break;
		}
	}
}